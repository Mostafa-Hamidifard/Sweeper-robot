...
class PositionControlWindow(Window):
    def __init__(self, title="position_control"):
        ...
        self.pid_slider = {key: sg.Slider((0, 1), size=(40, 20), orientation="horizontal", font=layout_config["font_small"], resolution=0.01) for key in ["kp", "ki", "kd"]}

        super().__init__(title)

    def _create_emergency_section(self):
        self.start_stop_btn = sg.Button("Start", size=(10, 1), font=layout_config["font_medium"])
        self.home_btn = sg.Button("Home", size=(10, 1), font=layout_config["font_medium"])
    ...
    def _create_brush_section(self):
        ...
        self.brush_speed_input = sg.Input(size=(5, 1), default_text="0", font=layout_config["font_medium"])
        self.brush_speed_set_btn = sg.Button("Set", key="BRUSH_SPEED_SET_BTN", size=(5, 1), font=layout_config["font_medium"])
        brush_layout = [
            [
    ...
    def _create_pid_tuning_section(self):
        pid_tune_layout = [
            [sg.Text("Kp:", size=(3, 1), font=layout_config["font_medium"]), self.pid_slider["kp"]],
            [sg.Text("Ki:", size=(3, 1), font=layout_config["font_medium"]), self.pid_slider["ki"]],
            [sg.Text("Kd:", size=(3, 1), font=layout_config["font_medium"]), self.pid_slider["kd"]],
        ]
    ...
    def _create_desired_position_section(self):
        self.des_pos = {
            "mode_radio": {
                "constant": sg.Radio("Constant", "desired_position_mode", font=layout_config["font_medium"]),
                "square": sg.Radio("Sqaure wave", "desired_position_mode", font=layout_config["font_medium"]),
                "sine": sg.Radio("Sine wave", "desired_position_mode", font=layout_config["font_medium"]),
            },
            "input": {
            ...
            "set_btn": {sg.Button("Set", "DES_POS_SET_BTN", size=(5, 1), font=layout_config["font_medium"])},
        }

        layout = [
            [self.des_pos["mode_radio"]["constant"], self.des_pos["mode_radio"]["square"], self.des_pos["mode_radio"]["sine"]],
            [
    ...
    def _create_frame_section(self):
        self.frame_pos = {
            "mode_radio": {
                "constant": sg.Radio("Constant", "desired_position_mode", font=layout_config["font_medium"]),
                "trapezoid": sg.Radio("Trapezoid wave", "desired_position_mode", font=layout_config["font_medium"]),
                "sine": sg.Radio("Sine wave", "desired_position_mode", font=layout_config["font_medium"]),
            },
            "input": {
                "amp": sg.Input("0", size=(5, 1), font=layout_config["font_medium"]),
            ...
            "set_btn": {sg.Button("Set", "DES_FRAME_SET_BTN", font=layout_config["font_medium"], size=(5, 1))},
        }

        layout = [
            [self.frame_pos["mode_radio"]["constant"], self.frame_pos["mode_radio"]["trapezoid"], self.frame_pos["mode_radio"]["sine"]],
            [
                sg.Text("Bias/Constant:", font=layout_config["font_small"]),
    ...
    def _create_save_section(self):
        self.save_btn = sg.Button("Save config", key="SAVE_CONFIG", font=layout_config["font_medium"])
        self.back_btn = sg.Button("Go to main Page", key="BACK", font=layout_config["font_medium"])
    ...
    def generate_layout(self) -> List:
        ...
        layout = [[sg.Column(column_layout_controls, vertical_alignment="top"), sg.Column(column_layout_figures)]]
